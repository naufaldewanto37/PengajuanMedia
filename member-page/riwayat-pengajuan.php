...
<head>
    ...
    <link rel="stylesheet" href="../style/style-riwayat.css">
...
<body>
    ...
    <div class="content">
        <div id="pilihan">
            <a href="riwayat-pengajuan.php" class="text-yellow" id="pengajuan">Pengajuan Saya</a>
        ...
        <div class="rectangle">
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM pengajuan JOIN member ON pengajuan.id_user = member.id_user WHERE pengajuan.id_user = ? ORDER BY pengajuan.tglaju DESC;";
                    if ($stmt = $conn->prepare($sql)) {
                        ...
                        while ($user = $result->fetch_assoc()) {
                            if ($user['status'] === 'DiSetuju') {
                                $colorClass = 'class-blue';
                            } elseif ($user['status'] === 'DiTolak') {
                                $colorClass = 'class-red';
                            } else {
                                $colorClass = 'class-black';
                            }
                            echo "<tr>
                                <td><a class='table-a $colorClass' href='tambah_pengajuan2.php?id_pengajuan=$user[id_pengajuan]'>$user[company_name]</a></td>
                                <td><a class='table-a $colorClass' href='tambah_pengajuan2.php?id_pengajuan=$user[id_pengajuan]'>$user[tglaju]</a></td>
                                <td><a class='table-a $colorClass' href='tambah_pengajuan2.php?id_pengajuan=$user[id_pengajuan]'>$user[status]</a></td>
                              </tr>";
                        }
                    } else {
